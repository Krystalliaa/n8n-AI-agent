...
KB Agent Master is an n8n Self-Hosted (v2.33.7) workflow acting as a Knowledge Base Auditor and Editor for the resume generation pipeline. It manages structured career data files at /data/resume-kb/ and enforces safe write operations through a two-step approval flow backed by a persistent state machine. The pipeline has two primary agents: KB Agent Master and Documentation Architect. KB Agent Master implements six features: Synonym Discovery (with LLM cache), Skill Gap Analysis (reads JDs from /data/resume-kb/jds/), Broken Link Detector (read-only), Resume Quality Audit (read-only), Test Suite (7/7 passing), and Supabase Integration (schema defined, data population pending). Safety architecture uses atomic writes, timestamped backups, append-only audit logs, and post-write YAML/JSON validation. Key constraints: n8n Code Node sandbox blocks require('js-yaml') unless NODE_FUNCTION_ALLOW_EXTERNAL is set or absolute path to node_modules is used; Chat Trigger executions are stateless so approval state must persist in /data/resume-kb/.state/session.json not /tmp/; LLM outputs require markdown code fence stripping before JSON.parse(); Execute Command stdout is truncated for large outputs so large data must be written to files; taxonomy_dictionary.json uses canonical_name as key not skill_id; Switch node rules must be manually verified after any workflow restructure; every branch in 'Using Response Nodes' mode must terminate with a Respond to Webhook node. Supabase Get Many node returns {data: [...]} not an array of n8n items.
